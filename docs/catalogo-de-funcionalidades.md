...
| Ceifa de CLI de agente órfão | Mata o processo de agente que sobreviveu ao runtime que o lançou, sem nunca tocar em processo alheio que herdou o mesmo PID | acionada por create/status/list/watch; `orphan_agent_reap_after_s` | Interna | `TaskService._reap_orphan_agents` e `_register_process_tracking` em `runtime/src/orchestrator_runtime/tasks/service.py:1675-1820`; `process_identity`/`identity_matches` em `runtime/src/orchestrator_runtime/agents/reaper.py:50-81`; `AgentProcessRow` em `runtime/src/orchestrator_runtime/memory/database.py:113-140` |
